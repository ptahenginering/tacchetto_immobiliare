...
| T39 — Deploy | ✅ | Workflow GitHub Actions con 4 job FTP SiteGround (sito vetrina, /app, /admin, /api con composer no-dev + .env dai secrets + migrazioni remote + health check); run-migrations.php protetto da MIGRATION_KEY con hash_equals; .htaccess backend/SPA; docker-compose per sviluppo locale; docs/deploy.md con secrets, cron e alternativa VPS nginx (cfr. DECISIONS D02) |
| T40 — Consegna | ✅ | README finale con avvio in 5 comandi, architettura, test, scheduler e CHECKLIST CHIAVI completa per la produzione |
